  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Card className="border-destructive/50 bg-destructive/5"><CardContent className="p-4 flex items-center gap-4"><div className="w-10 h-10 rounded-lg bg-destructive/10 flex items-center justify-center"><AlertTriangle className="w-5 h-5 text-destructive" /></div><div><p className="text-2xl font-bold text-foreground">{highRiskCount}</p><p className="text-sm text-muted-foreground">{t('students.highRisk')}</p></div></CardContent></Card>
        <Card className="border-orange-500/50 bg-orange-50 dark:bg-orange-950/20"><CardContent className="p-4 flex items-center gap-4"><div className="w-10 h-10 rounded-lg bg-orange-500/10 flex items-center justify-center"><AlertCircle className="w-5 h-5 text-orange-600 dark:text-orange-400" /></div><div><p className="text-2xl font-bold text-foreground">{mediumRiskCount}</p><p className="text-sm text-muted-foreground">{t('students.mediumRisk')}</p></div></CardContent></Card>
                    ...
                    <table className="w-full">
                      ...
                      <tbody>
                        {paginatedStudents.map((student: any) => {
                          ...
                          return (
                            <tr key={student.id} className={cn("border-b border-border/50 hover:bg-muted/50 transition-colors", isAtRisk && studentRisk?.riskLevel === 'high' && 'bg-destructive/5', isAtRisk && studentRisk?.riskLevel === 'medium' && 'bg-orange-50 dark:bg-orange-950/10', isAtRisk && studentRisk?.riskLevel === 'low' && 'bg-yellow-50 dark:bg-yellow-950/10')}>
                              <td className="py-3 px-4"><div className="flex items-center gap-3"><Avatar className="w-8 h-8"><AvatarFallback className={cn("text-sm", isAtRisk && studentRisk?.riskLevel === 'high' && 'bg-destructive/20 text-destructive', isAtRisk && studentRisk?.riskLevel === 'medium' && 'bg-orange-500/20 text-orange-600', isAtRisk && studentRisk?.riskLevel === 'low' && 'bg-yellow-500/20 text-yellow-600', !isAtRisk && 'bg-secondary/20 text-secondary')}>{student.profile?.name?.charAt(0) || 'S'}</AvatarFallback></Avatar><span className="font-medium text-foreground">{student.profile?.name || 'Unknown'}</span></div></td>
                              ...
                              <td className="py-3 px-4 text-center">{isAtRisk ? <Badge variant={studentRisk?.riskLevel === 'high' ? 'destructive' : 'outline'} className={cn('text-xs', studentRisk?.riskLevel === 'medium' && 'border-orange-500 text-orange-600', studentRisk?.riskLevel === 'low' && 'border-yellow-500 text-yellow-600')}>{studentRisk?.riskLevel} {t('students.risk')}</Badge> : <Badge variant="outline" className="text-xs border-green-500 text-green-600">{t('students.onTrack')}</Badge>}</td>
                            </tr>
                          );
                        })}
                      </tbody>
